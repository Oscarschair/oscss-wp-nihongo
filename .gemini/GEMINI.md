...
### 4. 過去記事重複ゼロ原則 ＆ theme-planner サブエージェント運用 (Zero-Duplication & Theme-Planner)
- **過去全記事の重複事前監査の義務化**:
  - 新規テーマ案の提案時は、必ずサブエージェント `theme-planner`（または `python scripts/core/audit_topic_duplicates.py "<キーワード>"`）を稼働させ、過去全記事（公開済み＋予約済み）のタイトル・文法項目・街角シーン・カルチャーショック事象との被り・カニバリゼーションがないことを客観的に証明すること。
- **ライフサイクルフック紐付け**:
  - テーマ提案トリガー（`topic_proposal`）に `theme-planner` が自動紐付けられ、提案前に全記事走査が実行される。
...
3. **4大カテゴリー別完全統一規格の構成要素（必須照合）**:
    - 🏪 **街角サバイバル【RPGバトルコマンド型】**:
      - 正本: 	humb_street_cafe_order_rpg.jpg / 	humb_street_onsen_sento_rpg.jpg
      - 上部中央白地カード（薄黄色「🏪 街角サバイバル」ピルタグ＋極太見出し＋サブ見出し）＋ 左側勇者クルマ（剣＆盾）＋ 右側笑顔の店員・NPC吹き出し ＋ 下部青色レトロRPGコマンドウィンドウ（▶たたかう/どうぐ/にげる）。
...
5. **記事内挿入図（オスカー登場）の必須配置原則（今後全記事に最低1枚適用）**:
    ...
    - シャンプー台の不透明な布（眼鏡の上から被せる）、改札の後ろの列、音姫のタイマーなど、生活実感・文化的整合性を徹底する。
---
